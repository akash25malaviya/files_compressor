...
const Video = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await AxiosInterceptor.post("video/", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
      if (response.status === 200) {
        const compressedPDFUrl = response.data.compressed_video;
        ...
        if(response?.data?.file?.length > 0){
          ...
          setErrorMessage("Sorry, Your Video is already very well compressed.");
        }
      }
    } catch (error) {
      ...
      <div style={{ marginTop: "35px", marginBottom: "35px" }}>
        <Box sx={{ marginY: "45px", paddingY: "20px", background: "#EEEDEB" }}>
          <Container maxWidth="lg">
            <Typography
              sx={{ textAlign: "center", fontWeight: "700", fontSize: "36px" }}
            >
